...
class UNet(nn.Module):
    def __init__(self, block=ConvBlock, dim=32, output_ch=3):
        ...
        self.conv10 = nn.Conv2d(dim, output_ch, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        out = self.activation(conv10)

        return [conv6, conv7, conv8, conv9], out
...
class Uformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        pool0 = self.dowsample_0(conv0, x_size)
        x_size = (x_size[0] // 2, x_size[1] // 2)
        conv1 = self.encoderlayer_1(pool0, x_size, mask=mask)
        pool1 = self.dowsample_1(conv1, x_size)
        x_size = (x_size[0] // 2, x_size[1] // 2)
        conv2 = self.encoderlayer_2(pool1, x_size, mask=mask)
        pool2 = self.dowsample_2(conv2, x_size)
        x_size = (x_size[0] // 2, x_size[1] // 2)
        conv3 = self.encoderlayer_3(pool2, x_size, mask=mask)
        pool3 = self.dowsample_3(conv3, x_size)
        x_size = (x_size[0] // 2, x_size[1] // 2)

        # Bottleneck
        conv4 = self.conv(pool3, x_size, mask=mask)

        # Decoder
        up0 = self.upsample_0(conv4, x_size)
        x_size = (x_size[0] * 2, x_size[1] * 2)
        deconv0 = torch.cat([up0, conv3], -1)
        deconv0 = self.decoderlayer_0(deconv0, x_size, mask=mask)

        up1 = self.upsample_1(deconv0, x_size)
        x_size = (x_size[0] * 2, x_size[1] * 2)
        deconv1 = torch.cat([up1, conv2], -1)
        deconv1 = self.decoderlayer_1(deconv1, x_size, mask=mask)

        up2 = self.upsample_2(deconv1, x_size)
        x_size = (x_size[0] * 2, x_size[1] * 2)
        deconv2 = torch.cat([up2, conv1], -1)
        deconv2 = self.decoderlayer_2(deconv2, x_size, mask=mask)

        up3 = self.upsample_3(deconv2, x_size)
        x_size = (x_size[0] * 2, x_size[1] * 2)
        deconv3 = torch.cat([up3, conv0], -1)
        deconv3 = self.decoderlayer_3(deconv3, x_size, mask=mask)

        # Output Projection
        y = self.output_proj(deconv3, x_size)
        y = self.activation(y)
        return y
